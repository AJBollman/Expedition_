...
public sealed class S_Drawing : MonoBehaviour
{
    ...
    private LineVertex newVert;
    public void placeVertex() {
        if(!_allowCameraDrawing) return; // TODO give some feedback to the player
        newVert = LineVertex.SpawnVertex(_goalIndicatorPos, _goalIndicatorRot);
    ...
    public void connectVertex() {
        if(!_allowCameraDrawing) return; // TODO give some feedback to the player
        if(_LastVertex != null) LineVertex.connectVertices(_LastVertex, newVert);
        _LastVertex = newVert;
    }
    #endregion
